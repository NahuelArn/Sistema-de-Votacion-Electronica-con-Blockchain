...
mod sistema_votacion {
    ...
    impl SistemaVotacion {
        ...
        pub fn get_peticiones_de_registro_sistema(&self) -> Result<Vec<Usuario>, ErrorSistema> {
            self.validar_permisos(
                Self::env().caller(),
                "Sólo el administrador puede ver las peticiones de registro.".to_owned(),
            )?;
        ...
        #[ink(message)]
        pub fn aprobar_usuario_sistema(
            &mut self,
            usuar_account_id: AccountId,
        ) -> Result<(), ErrorSistema> {
            self.validar_permisos(
                Self::env().caller(),
                "Sólo el administrador puede aprobar peticiones de usuarios para registro."
                    .to_owned(),
            )?;
        ...
        #[ink(message)]
        pub fn delegar_admin(&mut self,nuevo_admin_acc_id: AccountId,
            nuevo_admin_nombre: String,
            nuevo_admin_dni: String,
        ) -> Result<(), ErrorSistema> {
            self.validar_permisos(
                Self::env().caller(),
                "Sólo el administrador puede delegarle su rol a otra cuenta.".to_owned(),
            )?;

            self.corregir_estado_nuevo_admin(
                nuevo_admin_acc_id,
                nuevo_admin_nombre,
                nuevo_admin_dni,
            );
        ...
        pub fn crear_nueva_eleccion(
            ...
            self.validar_permisos(
                Self::env().caller(),
                "Sólo el administrador puede crear nuevas elecciones.".to_owned(),
            )?;
        ...
        {
            let caller_user = self.validar_caller_como_usuario_aprobado(
                Self::env().caller(),
                "Sólo los usuarios pueden registrarse a las elecciones.".to_owned(),
            )?;
        ...
        #[ink(message)]
        pub fn get_candidatos_pendientes(
            &mut self,
            eleccion_id: u64,
        ) -> Result<Vec<Usuario>, ErrorSistema> {
            ...
            Ok(self.elecciones[eleccion_index]
                .peticiones_candidatos
                .clone())
        }

        #[ink(message)]
        pub fn aprobar_candidato_eleccion(
            &mut self,
            eleccion_id: u64,
            ...
            self.validar_permisos(
                Self::env().caller(),
                "Sólo el administrador puede aprobar candidatos para las elecciones.".to_owned(),
            )?;
            ...
            let candidato_index =
                self.validar_candidato_en_pendientes(candidato_dni, eleccion_index)?;
        ...
        pub fn votar_eleccion(
            ...
            let caller_user = self.validar_caller_como_usuario_aprobado(
                Self::env().caller(),
                "Sólo los usuarios pueden votar una elección.".to_owned(),
            )?;
        ...
        fn corregir_estado_nuevo_admin(
            &mut self,
            new_admin_id: AccountId,
            new_admin_nombre: String,
            new_admin_dni: String,
        ) {
            if self
                .get_usuario_registrado_en_sistema(new_admin_id)
                .is_some()
            {
                return;
            }
            ...
            if self
                .get_usuario_en_peticiones_del_sistema(new_admin_id)
                .is_none()
            {
        ...
        fn registrar_peticion_eleccion(
            &mut self,
            user: Usuario,
            rol: Rol,
            eleccion_index: usize,
        ) -> Result<(), ErrorSistema> {
            self.validar_inexistencia_de_usuario_en_eleccion(
                user.account_id,
                rol.clone(),
                eleccion_index,
            )?;
        ...
        fn registrar_voto_a_candidato(
            &mut self,
            candidato_index: usize,
            eleccion_index: usize,
        ) -> Result<(), ErrorSistema> {
            if self.elecciones[eleccion_index].votos[candidato_index]
                .votos_recaudados
                .checked_add(1)
                .is_none()
            {
                return Err(ErrorSistema::RepresentacionLimiteAlcanzada {
                    msg: "Se alcanzó el límite de representación para este voto.".to_owned(),
                });
            }
        ...
        fn validar_permisos(
            ...
            if !self.es_admin(caller_id) {
                return Err(ErrorSistema::NoSePoseenPermisos { msg: err_msg });
            }
            Ok(())
        }

        fn es_admin(&self, caller_id: AccountId) -> bool {
            caller_id == self.admin_id
        }

        fn validar_caller_como_admin_o_usuario_aprobado(
            &self,
            caller_id: AccountId,
        ) -> Result<(), ErrorSistema> {
            if self.es_admin(caller_id) {
        ...
        fn validar_caller_como_usuario_aprobado(
            &self,
            caller_id: AccountId,
            admin_err_msg: String,
        ) -> Result<Usuario, ErrorSistema> {
            if self.es_admin(caller_id) {
                return Err(ErrorSistema::AccionUnicaDeUsuarios { msg: admin_err_msg });
            }
        ...
        fn consultar_inexistencia_usuario_en_sistema(
            &self,
            caller_id: AccountId,
        ) -> Result<(), ErrorSistema> {
            if self.es_admin(caller_id) {
                return Err( ErrorSistema::UsuarioYaRegistrado { msg: "Los administradores se registran al momento de instanciar el sistema, ó de delegar su rol.".to_owned() } );
            }

            if self.existe_usuario_en_peticiones_del_sistema(caller_id) {
                return Err( ErrorSistema::UsuarioYaRegistradoEnPeticiones { msg: "El usuario ya se encuentra registrado en la cola de aprobación del sistema, deberá esperar a ser aprobado.".to_owned() } );
            }
            if self.existe_usuario_registrado_en_sistema(caller_id) {
                return Err(ErrorSistema::UsuarioYaRegistrado {
                    msg: "El usuario ya se encuentra registrado y aprobado en el sistema"
                        .to_owned(),
                });
            }
        ...
        fn consultar_peticion_sistema(&self, user_id: AccountId) -> Result<(), ErrorSistema> {
            if self.existe_usuario_en_peticiones_del_sistema(user_id) {
                return Ok(());
            }
            ...
            return match self.existe_usuario_registrado_en_sistema(user_id) {
                ...
                false => Err(ErrorSistema::NoExisteUsuario {
                    msg: "El usuario no existe en el sistema.".to_owned(),
                }),
            };
        }
        ...
        fn get_usuario_registrado_en_sistema(&self, user_id: AccountId) -> Option<usize> {
            for i in 0..self.usuarios_registados.len() {
                if self.usuarios_registados[i].account_id == user_id {
                    return Some(i);
                }
            }
        ...
        fn get_usuario_en_peticiones_del_sistema(&self, user_id: AccountId) -> Option<usize> {
            for i in 0..self.peticiones_registro.len() {
                if self.peticiones_registro[i].account_id == user_id {
                    return Some(i);
                }
            }
        ...
        fn validar_usuario_en_sistema(&self, caller_id: AccountId) -> Result<usize, ErrorSistema> {
            if let Some(index) = self.get_usuario_registrado_en_sistema(caller_id) {
                return Ok(index);
            }
        ...
        fn check_add_elecciones_id(&mut self) -> Result<(), ErrorSistema> {
            ...
            if result.is_none() {
                return Err( ErrorSistema::RepresentacionLimiteAlcanzada { msg: "La máxima representación del tipo de dato fue alcanzada. Mantenimiento urgente.".to_owned() } );
            }
        ...
        fn existe_eleccion(&self, eleccion_id: u64) -> Result<usize, ErrorSistema> {
            if eleccion_id >= self.elecciones_conteo_id {
                return Err(ErrorSistema::ErrorDeEleccion {
                    error: ErrorEleccion::NoExisteEleccion {
                        msg: "La id de elección ingresada no existe.".to_owned(),
                    },
                });
            }
        ...
        fn validar_inexistencia_de_usuario_en_eleccion(
            &self,
            caller_id: AccountId,
            rol: Rol,
            eleccion_index: usize,
        ...
        fn validar_votante_aprobado_en_eleccion(
            &self,
            votante_id: AccountId,
            eleccion_index: usize,
        ) -> Result<(), ErrorSistema> {
            if self.elecciones[eleccion_index]
                .votantes_aprobados
                .iter()
                .any(|v| v.account_id == votante_id)
            {
                return Ok(());
            }
        ...
        fn validar_candidato_en_pendientes(
            &self,
            candidato_dni: String,
            eleccion_index: usize,
        ) -> Result<usize, ErrorSistema> {
            if let Some(index) = self.get_candidato_pendiente(candidato_dni.clone(), eleccion_index)
            {
                return Ok(index);
            }

            return match self
                .get_candidato_aprobado(candidato_dni, eleccion_index)
                .is_some()
            {
                true => Err(ErrorSistema::ErrorDeEleccion {
                    error: ErrorEleccion::CandidatoActualmenteAprobado {
                        msg: "El candidato ingresado ya se encuentra actualmente aprobado."
                            .to_owned(),
                    },
                }),
                false => Err(ErrorSistema::ErrorDeEleccion {
                    error: ErrorEleccion::CandidatoNoExiste {
                        msg: "El candidato ingresado no existe en la elección.".to_owned(),
                    },
                }),
            };
        }

        fn validar_candidato_aprobado(
            &self,
            candidato_dni: String,
            eleccion_index: usize,
        ) -> Result<usize, ErrorSistema> {
            if let Some(index) = self.get_candidato_aprobado(candidato_dni.clone(), eleccion_index)
            {
                return Ok(index);
            }

            return match self
                .get_candidato_pendiente(candidato_dni, eleccion_index)
                .is_some()
            {
                true => Err(ErrorSistema::ErrorDeEleccion {
                    error: ErrorEleccion::CandidatoNoAprobado {
                        msg: "El candidato ingresado está en espera de aprobación.".to_owned(),
                    },
                }),
                false => Err(ErrorSistema::ErrorDeEleccion {
                    error: ErrorEleccion::CandidatoNoExiste {
        ...
        fn get_candidato_aprobado(
            &self,
            candidato_dni: String,
            eleccion_index: usize,
        ) -> Option<usize> {
            for i in 0..self.elecciones[eleccion_index].candidatos_aprobados.len() {
                if self.elecciones[eleccion_index].candidatos_aprobados[i].dni == candidato_dni {
                    return Some(i);
                }
            }
    ...
    impl Eleccion {
        ...
        fn get_estado_eleccion(&self, timestamp: u64) -> EstadoEleccion {
            ...
            if timestamp < f_ini {
                estado = EstadoEleccion::PeriodoInscripcion;
            } else if (timestamp > f_ini) && (timestamp < f_crr) {
                estado = EstadoEleccion::PeriodoVotacion;
            } else {
    ...
    pub struct Fecha {
        ...
        min: u8,
        seg: u8,
    } // Año como unsigned debido a que sí o sí se tratarán fechas mayores a la actual
